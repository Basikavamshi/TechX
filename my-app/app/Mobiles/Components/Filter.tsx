...
function Filter({onClose}:{onClose:()=>void}) {
    ...
    const PriceRange=["Under $10,000","10,000-20,000","20,000-30,000","Above $30,000"]
            ...
            <div className='grid grid-cols-[repeat(2,minmax(120px,1fr))] grid-flow-row w-full h-full gap-2 justify-center itmes-center pl-2'>
                {
                    trending.map((value,id)=>{
                        return(
                            <button key={id} className='flex justify-center items-center w-full h-fit text-center border rounded-2xl box-border p-1 text-[0.8em] wrap-normal'>{value}</button>
                        )
                    })
                }
            ...
            <div className='flex flex-col justify-start items-center gap-2 pl-2'>
               {
                TopBrands.map((value,key)=>{
                    return(
                        <div key={key} className='flex flex-row justify-start items-center gap-2 w-full h-full '>
